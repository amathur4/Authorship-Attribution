...
for count in range(len(a)):
    ...
    stt = 'select * from reviews where customer_id!="'+a[count]+'"  limit '+ str(len(df0)) 
    cursor.execute(stt)
    rows = cursor.fetchall()
    ...
    #additional features
    X=np.column_stack((punct_count,flesch_reading_ease1,difficult_words1,smog_index1,automated_readability_index1,coleman_liau_index1,linsear_write_formula1,dale_chall_readability_score1,avg_word_freq,capital_count))
